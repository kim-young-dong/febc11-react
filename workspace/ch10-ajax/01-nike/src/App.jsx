...
function App() {
  ...
  const fetchData = async (_id) => {
    try {
      const response = await fetch(`https://11.fesp.shop/products/${_id}`, {
        method: "GET",
        headers: {
          "client-id": "vanilla01",
        },
      ...
      setData(data.item);
      data.shippingFees && setShippingFees(data.shippingFees);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleQuantityChange = (e) => {
    ...
    setQuantity(value);
    setShippingFees(Math.ceil(value / 5) * shippingFees);
  };
      ...
      {data && (
        <>
          ...
          <div>
            ...
            />
            (배송비는 5개당 {(shippingFees || 3000).toLocaleString()}원씩
            추가됩니다.)
            <br />
